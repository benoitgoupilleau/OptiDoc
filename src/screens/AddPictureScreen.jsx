...
const AddPictureScreen = React.memo(({ navigation, user, userBusiness, editFile, addNewDoc }) => {
  ...
  const onCreatePicture = () => {
    ...
    RNImageToPdf.createPDFbyImages({ imagePaths: [path], name: 'fileID' })
      .then((file) =>
        RNFS.mkdir(`${rootDir}/${user.userId}/${businessId}/${Folder.rea}`).then(() =>
    ...
    <Container>
      ...
      <Wrapper>
        ...
        <View>
          ...
          <Comment multiline={true} onChangeText={(comment) => setComment(comment)} value={comment} returnKeyType="done" />
          ...
          <StyledScroll width={width} height={height}>
            {uri ? (
              <PictureWrapper>
                <ImageFrame source={{ uri: uri }} screenWidth={width} width={width} height={height} />
                <Icons color="green" name="md-checkmark-circle" size={Layout.icon.default} />
              </PictureWrapper>
            ) : (
